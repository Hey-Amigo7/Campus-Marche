...
export default function OrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [buyerCoords, setBuyerCoords] = useState<DeliveryCoords | null>(
    order?.tracking?.buyerLatitude != null
      ? { lat: order.tracking.buyerLatitude, lng: order.tracking.buyerLongitude! }
      : null
  );
  ...
  // Sync delivery and buyer coords from fresh SWR order data
  useEffect(() => {
    ...
    if (order.tracking.buyerLatitude != null) {
      setBuyerCoords({ lat: order.tracking.buyerLatitude, lng: order.tracking.buyerLongitude! });
    }
  ...
  // Buyer sees confirm button when delivery has started
  const showOldConfirmButton = role === "buyer" && isOutForDelivery;
  ...
  async function handleAssignDelivery(e: FormEvent) {
    ...
    try {
      ...
      await mutate();
      toast("Delivery person assigned.");
      setDeliveryContact("");
              ...
              <section className="rounded-2xl p-5" style={{ background: "rgba(255,255,255,0.82)", backdropFilter: "blur(18px)", border: "1px solid rgba(226,232,240,0.70)", boxShadow: "0 4px 24px rgba(15,23,42,0.07)" }}>
                ...
                {role === "buyer" ? (
                  ...
                  <div className="mt-3 space-y-3">
                    ...
                    {/* Show current delivery person assignment with option to remove (before delivery starts) */}
                    {(order.deliveryPersonId || (order as { externalDeliveryContact?: string }).externalDeliveryContact) && order.status === "In progress" ? (
