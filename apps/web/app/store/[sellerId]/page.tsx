      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="bg-brand-navy p-6 text-white md:p-10">
          <div className="grid gap-6 md:grid-cols-[1fr_auto] md:items-end">
            <div className="flex items-center gap-4">
              <span className="grid h-20 w-20 place-items-center rounded-2xl bg-white text-lg font-black text-brand-navy">
              ...
              <div>
                ...
                <p className="mt-2 flex items-center gap-2 font-semibold text-blue-100">
                  <MapPin className="h-4 w-4" />
                  {seller.location} · Joined {seller.joined}
                </p>
              </div>
            </div>
            <Link href="/messages" className="btn-primary bg-brand-green hover:bg-green-700">
        ...
        <div className="grid gap-6 p-6 md:grid-cols-[1fr_340px] md:p-8">
          <div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h2 className="text-xl font-black text-slate-950">{seller.banner}</h2>
                  <p className="mt-2 leading-7 text-slate-600">{seller.bio}</p>
                </div>
              </div>
            </div>
            {seller.analytics ? (
          ...
          <aside className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <h3 className="font-black text-slate-950">Seller trust</h3>
            <div className="mt-4 space-y-4">
              <Rating value={seller.rating} reviews={seller.reviews} />
              {seller.responseTime ? (
                <p className="text-sm font-semibold text-slate-500">{seller.responseTime}</p>
              ) : null}
              {sellerProducts != null ? (
      ...
      <section className="mt-10">
        ...
        {sellerProducts && sellerProducts.length > 0 ? (
          ...
          <EmptyState
            ...
            description="This seller hasn't posted any products yet."
          />
        )}
      </section>
    </div>
